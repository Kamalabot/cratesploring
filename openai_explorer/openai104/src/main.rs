...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = CreateChatCompletionRequestArgs::default()
        ...
        .messages([ChatCompletionRequestUserMessageArgs::default()
            ...
            .build()?
            .into()])
        // Using the tool completion here
        .tools(vec![ChatCompletionToolArgs::default()
            ...
            .build()?])
        .build()?;
    // we have to build the function, tool args and then
    // tool request

    let response_message = client
        .chat()
        .create(request) // tool request is used
        .await?
        .choices
        .first() // first of the responses
        .unwrap()
        .message // message extracted
        .clone();
    // multiple tools are called from the response
    if let Some(tool_calls) = response_message.tool_calls {
        ...
        for tool_call in tool_calls {
            ...
            // okay the tools are called in parallel here
            // to the OpenAI API
            let handle =
        ...
        for (handle, tool_call_clone) in handles {
            // each of the handle is awaited below
            if let Ok(response_content) = handle.await {
                // the responses are pushed into the vectors
